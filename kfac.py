...
class KfacOptimizer():
    ...
    def __init__(self, learning_rate=0.01, momentum=0.9, clip_kl=0.01, upper_bound_kl=False, kfac_update=20, stats_accum_iter=60, full_stats_init=False, cold_iter=100, cold_lr=None, async=False, async_stats=False, epsilon=1e-2, stats_decay=0.95, blockdiag_bias=False, channel_fac=False, factored_damping=False, approxT2=False, use_float64=False, weight_decay_dict={}):
        ...
        self._upper_bound_kl = upper_bound_kl
        ...
        def searchFactors(gradient, graph):
            ...
            if 'AddN' in bpropOp_name:
                ...
                if fpropOp.op_def.name in KFAC_OPS:
                    ...
                    bInputsList = [
                        i for i in bpropOp.inputs[0].op.inputs if 'gradientsSampled' in i.name if 'Shape' not in i.name]
            ...
            for i, param in enumerate(varlist):
                    ...
                    if not factorTensors[param].has_key(key + '_concat'):
                        ...
                        with tf.name_scope(name_scope):
                            factorTensors[param][key +
                                                 '_concat'] = tf.concat(0, factorTensors[param][key])
                else:
                ...
                for j, param2 in enumerate(varlist[(i + 1):]):
                    if (len(factorTensors[param][key]) > 0) and (set(factorTensors[param2][key]) == set(factorTensors[param][key])):
                        ...
                        factorTensors[param2][key +
                                              '_concat'] = factorTensors[param][key + '_concat']
        ########
            ...
            with tf.device('/cpu'):
                ...
                for var in varlist:
                    ...
                    if fpropFactor is not None:
                        if not tmpStatsCache.has_key(fpropFactor):
                            if opType == 'Conv2D':
                                Kh = var.get_shape()[0]
                                Kw = var.get_shape()[1]
                                C = fpropFactor.get_shape()[-1]
                    ...
                    if bpropFactor is not None:
                        ...
                        if not((not self._blockdiag_bias) and self.stats[var]['assnWeights']):
                            if not tmpStatsCache.has_key(bpropFactor):
    ...
    def compute_stats(self, loss_sampled, var_list=None):
        ...
        for var in varlist:
            ...
            for stats_var in fpropStats_vars:
                ...
                if not statsUpdates_cache.has_key(stats_var):
                    ...
                    if opType == 'Conv2D':
                        ...
                        if Oh == 1 and Ow == 1 and self._channel_fac:
                            ...
                            patches = tf.extract_image_patches(fpropFactor, ksizes=[
                                                               1, convkernel_size[0], convkernel_size[1], 1], strides=strides, rates=[1, 1, 1, 1], padding=padding)
                    ...
                    if stats_var_dim == (fpropFactor_size + 1) and not self._blockdiag_bias:
                        if opType == 'Conv2D' and not self._approxT2:
                            ...
                            fpropFactor = tf.concat(1, [fpropFactor, tf.ones(
                                [tf.shape(fpropFactor)[0], 1]) / Oh / Ow])
                        else:
                            # use homogeneous coordinates
                            fpropFactor = tf.concat(
                                1, [fpropFactor, tf.ones([tf.shape(fpropFactor)[0], 1])])
                ...
                for var in stats:
                    for key in ['fprop_concat_stats', 'bprop_concat_stats']:
                        for stats_var in stats[var][key]:
                            if not tmpEigenCache.has_key(stats_var):
        ...
        with tf.device('/cpu:0'):
            ...
            def copyStats(var_list):
                ...
                for item in var_list:
                    if item is not None:
                        if not redundant_stats.has_key(item):
            ...
            with tf.control_dependencies([]):
                for stats_var in stats_eigen:
                    if not computedEigen.has_key(stats_var):
    ...
    def getKfacPrecondUpdates(self, gradlist, varlist):
        ...
        for grad, var in zip(gradlist, varlist):
            ...
            if (len(fpropFactoredFishers) + len(bpropFactoredFishers)) > 0:
                ...
                if (self.stats[var]['assnBias'] is not None) and not self._blockdiag_bias:
                    ...
                    grad = tf.concat(
                        0, [grad, tf.expand_dims(grad_dict[var_assnBias], 0)])
        ...
        for g, var in zip(gradlist, varlist):
            ...
            #local_vg = tf.reduce_sum(grad * g)  # CHANGED MAJOR
            vg += local_vg
            ...
            with tf.control_dependencies(dependency_list):
                ...
                with tf.control_dependencies([updateFactorOps]):
                    ...
                    def optimOp():
                        def updateOptimOp():
                            if self._full_stats_init:
                                return tf.cond(tf.greater(self.factor_step, tf.convert_to_tensor(0)), lambda: optim.apply_gradients(zip(u, varlist)), tf.no_op)
                            else:
                                return optim.apply_gradients(zip(u, varlist))
                        if self._full_stats_init:
